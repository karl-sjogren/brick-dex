...
public class CallbackModel : PageModel {
    ...
    public async Task<IActionResult> OnGetAsync(
        ...
        // Authenticate against the external cookie scheme where OAuth handlers store results
        var authenticateResult = await HttpContext.AuthenticateAsync(AuthenticationExtensions.ExternalScheme);
        ...
        var picture = externalPrincipal.FindFirst("urn:google:picture")?.Value
            ...
            ?? externalPrincipal.FindFirst("urn:facebook:picture")?.Value
            ?? externalPrincipal.FindFirst("urn:twitch:profile_image_url")?.Value
            ?? externalPrincipal.FindFirst("urn:discord:avatar")?.Value;

        if(string.IsNullOrEmpty(providerKey) || string.IsNullOrEmpty(email)) {
            _logger.LogWarning(
                "Could not retrieve user information from {Provider}",
                provider);
            return RedirectToPage("/Account/Login", new {
                error = "Could not retrieve user information from the provider."
            });
        }

        // Get invite code from auth properties (stored by Login page)
        string? inviteCode = null;
        authenticateResult.Properties?.Items.TryGetValue("invite", out inviteCode);
        ...
        if(requiresInvite) {
            _logger.LogWarning(
                "User {Email} attempted registration without valid invite",
                email);
        ...
        // Clean up the external cookie - no longer needed
